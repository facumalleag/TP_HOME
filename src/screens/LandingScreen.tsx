...
export const LandingScreen = ({ navigation }: Props) => {
  ...
  const handleFilterChange = (filters: string[]) => {
    ...
    // Haz lo que necesites con los nuevos filtros
  };
  return (
    <View style={styles.root}>
      <FilterBar filters={["+ 3 Amb", "+ 2 Amb", "+ 2 Baños", "+ 1 Cochera"]} onFilterChange={handleFilterChange} />
      <TouchableOpacity onPress={() => navigation.navigate('LoginGeneralScreen')}>
        <Image source={{ uri: imagen_logo }} style={{ width: 300, height: 200, alignSelf: 'center' }} />
        <Text style={{ ...styles.myHome, alignSelf: 'center' }}>MyHome</Text>
        <Image
          source={{ uri: imagen_bienvenida }}
          style={{
            width: 400,
            height: 420,
            alignSelf: 'center',
